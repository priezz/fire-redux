...
export async function uploadSingleFile(f: any, writePath: string, existingMedia?: any[]) {
    ...
    let base64Header
    if(!base64) {
        const result = (await fileToBase64Helper(rawFile, uri, url)).split('base64,')
...
export async function uploadAttachments(
    ...
    // console.debug('FirebaseStorage/uploadAttachments()', media)

    /* Only freshly dropped media are instance of File */
    const newPredicate = (m: any) => m.url
        && ((m.url.startsWith('file:///') && m.base64) || m.url.startsWith('blob:'))
